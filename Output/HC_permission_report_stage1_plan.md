# HC Permission + Investigation Report — JMeter 测试计划

## 1. 输入与负载模型

- 抓包：`Fiddler file/HC_permission_report.saz`（75 个 HTTP 会话）。
- 目标：`https://hcrms.csitech.com:443`。
- 线程数：1；Ramp-up：1 秒；持续时间：600 秒。
- 线程组持续循环；登录和初始化仅在线程首次启动时执行一次。
- `case_id` 每轮从与 JMX 同目录的 `case id.csv` 读取。
...
```text
Test Plan — HC Permission and Investigation Report
└── Thread Group — HC Permission and Report Stable Load
    ├── CSV Data Set Config — case id.csv
    ├── HTTP Request Defaults — https / hcrms.csitech.com / 443
    ...
    │   │   ├── GET /RMS/AspSoft/Security/CasePermission.aspx
    │   │   └── POST /RMS/AspSoft/Security/CasePermission.aspx
    │   └── Transaction Controller — Add investigation report
    │       ├── GET /RMS/Aspsoft/Dispatcher.aspx
    │       ├── POST /RMS/Aspsoft/Dispatcher.aspx
    │       ├── GET /RMS/aspsoft/popupdispatcher.aspx
    │       ├── POST /RMS/aspsoft/popupdispatcher.aspx
    │       ├── GET /RMS/Aspsoft/IntakeForm/middlepage.aspx
    │       ├── POST /RMS/Aspsoft/IntakeForm/middlepage.aspx
    │       ├── GET /RMS/Aspsoft/IntakeForm/Intake.aspx
    │       ├── POST /RMS/AspSoft/IntakeForm/Intake.aspx
    │       ├── GET /RMS/AspSoft/IntakeForm/IntakeReportAssignWorkflow.aspx
    │       ├── POST /RMS/AspSoft/IntakeForm/IntakeReportAssignWorkflow.aspx (选择下一步)
    │       ├── POST /RMS/AspSoft/IntakeForm/IntakeReportAssignWorkflow.aspx (clear)
    │       ├── POST /RMS/AspSoft/IntakeForm/IntakeReportAssignWorkflow.aspx (保存)
    │       └── GET /RMS/Aspsoft/Dispatcher.aspx
...
- NJS 固定使用抓包值：描述 `SHOPLIFTING 4TH DEGREE >$200< $75,000`，代码 `2C:20-11C(3)`。
...
- 在 `case id.csv` 的表头下补充有效 case ID，每行一个。
